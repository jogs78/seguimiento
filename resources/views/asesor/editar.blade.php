...
<div class="centro"; style="margin-top: 60px; ">
    <form action="{{route("asesores.update",$asesor->id)}}" method="POST" enctype="application/x-www-form-urlencoded" >
        ...
        {{$errors->first("nombre")}}
        <input type='text' name='nombre' id='nombre' value="{{$asesor->nombre}}" class="llenar"><br>

        <label for='apellido_paterno' class="parrafo">Apellido Paterno</label>
        {{$errors->first("apellido_paterno")}}
        <input type='text' name='apellido_paterno' id='apellido_paterno' value="{{$asesor->apellido_paterno}}" class="llenar"><br>

        <label for='apellido_materno' class="parrafo">Apellido Materno</label>
        {{$errors->first("apellido_materno")}}
        <input type='text' name='apellido_materno' id='apellido_materno' value="{{$asesor->apellido_materno}}" class="llenar"><br>

        <label for='correo_electronico' class="parrafo">Correo Electronico</label>
        {{$errors->first("correo_electronico")}}
        <input type='email' name='correo_electronico' id='correo_electronico' value="{{$asesor->correo_electronico}}" class="llenar"><br>

        <label for='profesion' class="parrafo">Profesion</label>
        {{$errors->first("profesion")}}
        <input type='text' name='profesion' id='profesion' value="{{$asesor->profesion}}" class="llenar"><br>

        <label for='carrera' class="parrafo">Carrera</label>
        {{$errors->first("carrera")}}
        <input type='text' name='carrera' id='carrera' value="{{$asesor->carrera}}" class="llenar"><br>

        <label for='numero_cedula' class="parrafo">Numero Cedula</label>
        {{$errors->first("numero_cedula")}}
        <input type='text' name='numero_cedula' id='numero_cedula' value="{{$asesor->numero_cedula}}" class="llenar"><br>

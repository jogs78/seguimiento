<template>
  <AppLayout>
      ...
      <div class="documentos-grid">
        ...
        >
          <div class="card-header">
            ...
            <h3>
              {{ tipo.nombre }}
              <span v-if="tipo.es_requerido" class="required">*</span>
            ...
            <div v-if="getDocumentoPorTipo(tipo.id)" class="documento-subido">
              <div class="info-documento">
                ...
                <div class="detalles">
                  ...
                  <span class="tamaño">{{ formatTamaño(getDocumentoPorTipo(tipo.id).tamano_bytes) }}</span>
                  <span class="fecha">Subido: {{ formatFecha(getDocumentoPorTipo(tipo.id).subido_en) }}</span>
                </div>
              </div>
    ...
    <div style="background: yellow; padding: 20px; margin: 10px; border: 3px solid red;">
      <h2 style="color: red;">DEBUG - ¿Llegan los datos?</h2>
      <p><strong>tiposDocumento:</strong> {{ tiposDocumento ? tiposDocumento.length : 'UNDEFINED' }}</p>
      <p><strong>documentos:</strong> {{ documentos ? documentos.length : 'UNDEFINED' }}</p>
      
      <div v-if="tiposDocumento && tiposDocumento.length > 0">
        <h3>Tipos de documento:</h3>
        <ul>
          <li v-for="t in tiposDocumento" :key="t.id">
      ...
      <div v-else>
        <p style="color: red;">⚠️ NO HAY DATOS EN tiposDocumento</p>
      </div>
      
      <hr>
      <p><strong>Props completas:</strong></p>
      <pre>{{ JSON.stringify($props, null, 2) }}</pre>
    </div>

    <div style="padding: 40px; text-align: center;">
      <h1>Página de Prueba</h1>
      <p>tiposDocumento: {{ tiposDocumento.length }}</p>
      <ul>
        <li v-for="t in tiposDocumento" :key="t.id">
          {{ t.id }} - {{ t.nombre }}
        </li>
